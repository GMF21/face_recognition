...
if not os.path.exists("modelo_faces.pkl"):
    print("❌ Erro: modelo_faces.pkl não encontrado!")
    print("Por favor, execute 'python train.py' primeiro para treinar o modelo.")
    print("Certifique-se de adicionar imagens na pasta 'imagens/' antes de treinar.")
    exit(1)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:  # Sair
                running = False
            elif event.key == pygame.K_SPACE:  # Verificar rosto
